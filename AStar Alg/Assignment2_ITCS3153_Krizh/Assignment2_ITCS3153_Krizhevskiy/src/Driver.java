 ...
 * 
 * *** the instructions state "Blocks (unpathable tiles) are randomly placed 
 * in 10% of the node (not hard coded)" i set it to 35% to make it more 
 * interesting. this can be changed in the fillEnviroment(int[][] x)
 * method by changing the ratio variable to .10 (10%) from .35 ***
 * 
...
public class Driver extends JFrame {
    ...
    private void fillEnviroment(int[][] x) {
        int size = x.length * x[0].length;//number of locations
        double ratio = .35;//ratio of unpathable tiles
        int count = (int) (size * ratio) + 1;//number of unpathable tiles to be generated
        //count keeps track of the number of non-transverable tiles that are needed
        //loop through array
        for (int i = 0; i < x.length; i++) {
            for (int j = 0; j < x[i].length; j++) {
                //random number between 0 and (size of array-1)
                int rand = (int) (Math.random() * size);
                //next if helps distribute the non-transverable locations evely
                if (count > 0 && rand < (int) (size * ratio)) {
                    x[i][j] = 1;
                    count--;
                    //System.out.println(count);
                } else {
                    x[i][j] = 0;
                    //if filling last rows increase probablity they will be filled    
                    if (i >= x.length - 2) {
                        ratio = .5;
                    }
                }
